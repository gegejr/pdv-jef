...
<div class="min-h-screen md:pl-64 transition-all duration-300">
    ...
    <div class="pt-[72px] p-6">
        ...
        <form id="filtroForm" method="GET" action="{{ route('painel') }}" class="bg-white p-6 rounded-xl shadow mb-8 flex flex-col sm:flex-row items-end gap-4 relative">
            <div>
                <label for="inicio" class="block text-sm font-semibold mb-1">Data Início</label>
                <input type="date" name="inicio" id="inicio" value="{{ $inicio ?? '' }}" class="w-full border-gray-300 rounded-md shadow-sm">
            </div>
            <div>
                <label for="fim" class="block text-sm font-semibold mb-1">Data Fim</label>
                <input type="date" name="fim" id="fim" value="{{ $fim ?? '' }}" class="w-full border-gray-300 rounded-md shadow-sm">
            </div>
            ...
            @endif
            <!-- Loading (inicialmente escondido) -->
            ...
            <div class="bg-gradient-to-r from-purple-50 to-purple-100 p-5 rounded-xl shadow hover:shadow-xl transition transform hover:scale-105 duration-300">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-purple-700 mt-2">R$ {{ number_format($vendasHoje, 2, ',', '.') }}</p>
                    </div>
                    <div class="flex items-center justify-center w-12 h-12 bg-purple-200/50 rounded-full">
