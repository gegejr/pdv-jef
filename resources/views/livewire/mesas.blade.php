        ...
        <div class="flex-1 p-6 ml-64 md:ml-0 transition-all duration-300">
            ...
            @foreach ($mesasFree as $mesa)
                <div class="p-4 border rounded shadow text-center">
                    <div class="text-xl font-semibold">#{{ $mesa->numero }}</div>
            ...
            <!-- ======= MESAS FINALIZADAS ======= -->
            <h3 class="font-bold mt-10">Mesas Finalizadas</h3>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            @foreach ($mesasDone as $mesa)
                <div class="p-4 border rounded shadow bg-gray-200 text-center">
                    <div class="text-xl font-semibold">#{{ $mesa->numero }}</div>
                    <span class="text-gray-600 text-sm block">aguardando pagamento</span>

                    <button wire:click="verDetalhes({{ $mesa->id }})"
                            class="bg-gray-700 text-white px-3 py-1 mt-2 rounded">
                        Ver detalhes
                    </button>
                </div>
            @endforeach
            ...
            <div class="bg-white w-full max-w-2xl p-6 rounded">
                ...
                <ul class="mb-4 space-y-1">
                    @foreach ($itensCarrinho as $item)
                        <li>{{ $item['qtd'] }} × {{ $item['produto']->nome }}</li>
                    @endforeach
                </ul>
